...
class _OrderDetailsDrawerState extends State<OrderDetailsDrawer> {
  ...
  Map<String, dynamic> _formatOrderDataForPrinting({
    ...
    String groupNameFromId(String? groupId) {
      if (groupId == null || groupId.isEmpty) return '';
      final groups = storeModifierGroups.where((g) => g.id == groupId);
      return groups.isNotEmpty ? groups.first.name : '';
    }

    String modifierGroupName(OrderModifier modifier) {
      final directGroupName = groupNameFromId(modifier.modifierGroupId);
      if (directGroupName.isNotEmpty) return directGroupName;
      if (modifier.id.isEmpty) return '';
      final resolved = storeModifiers.where((m) => m.id == modifier.id);
      if (resolved.isEmpty) return '';
      return groupNameFromId(resolved.first.modifierGroupId);
    }
