...
// Re-export cart data models for convenience
...
class CartDrawer extends StatefulWidget {
  ...
  final VoidCallback? onShiftTableTap; // For dine-in orders
  final VoidCallback? onShiftGuestTap; // For shifting items between guests
  final String? currentTableName; // Current table name for dine-in orders
  final VoidCallback? onPrintKitchen;
  final VoidCallback? onPrintCustomer;
  final VoidCallback? onPrintQuote;
  final VoidCallback? onPrintSplitReceipt; // For printing per guest/table
  final VoidCallback? onSendToKitchen;
  final VoidCallback? onCheckout;
  final VoidCallback? onClearCart;
  final Function(Order)? onVoidOrder;
  final Function(String)?
  onGuestGroupChanged; // Callback when guest group changes
  final String? selectedGuestGroup; // Current selected guest group from parent
  final int? partySize; // Number of guests for dine-in orders
  final DateTime? orderCreatedAt; // Order creation time for occupied duration
  ...
  const CartDrawer({
    ...
    this.onContactTap,
    this.onAddItemTap,
    this.onShiftTableTap,
    this.onShiftGuestTap,
    this.currentTableName,
    this.onPrintKitchen,
    this.onPrintCustomer,
    this.onPrintQuote,
    this.onPrintSplitReceipt,
    this.onSendToKitchen,
    this.onCheckout,
    this.onClearCart,
    this.onVoidOrder,
    this.onGuestGroupChanged,
...
class _CartDrawerState extends State<CartDrawer> {
  ...
  void initState() {
    ...
    if (widget.orderCreatedAt != null) {
      _occupiedTimeTimer = Timer.periodic(const Duration(minutes: 1), (_) {
        if (mounted) {
          setState(() {}); // Trigger rebuild to update occupied time
        }
      });
    }
  }
  ...
  void didUpdateWidget(CartDrawer oldWidget) {
    ...
    // Start timer if orderCreatedAt becomes available
    if (widget.orderCreatedAt != null && oldWidget.orderCreatedAt == null) {
      _occupiedTimeTimer = Timer.periodic(const Duration(minutes: 1), (_) {
        if (mounted) {
          setState(() {}); // Trigger rebuild to update occupied time
        }
    ...
    // Stop timer if orderCreatedAt becomes null
    else if (widget.orderCreatedAt == null &&
        oldWidget.orderCreatedAt != null) {
      _occupiedTimeTimer?.cancel();
      _occupiedTimeTimer = null;
    }
  }
  ...
  // Get the currently selected guest group (from parent if available)
  String get _selectedGuestGroup => widget.selectedGuestGroup ?? 'whole_table';

  // Get number of guests from table info (partySize)
  int get _guestCount {
    return widget.partySize ?? 4; // Default to 4 if not provided
  }

  // Calculate occupied time
  String get _occupiedTime {
    if (widget.orderCreatedAt == null) return '0m';
    final duration = DateTime.now().difference(widget.orderCreatedAt!);
  ...
  List<CartItem> _getItemsForGuest(String guestGroup) {
    return widget.cartItems
        .where(
          (item) =>
              item.itemStatus != 'Voided' && item.guestGroup == guestGroup,
        )
        .toList();
  }

  void _handleSectionTap(String guestId) {
    setState(() {
      // If clicking the already selected section, just toggle expand/collapse
      if (_selectedGuestGroup == guestId) {
        if (_expandedSections.contains(guestId)) {
          _expandedSections.remove(guestId);
        } else {
          _expandedSections.add(guestId);
        }
      } else {
        // If clicking a different section, select it and expand it
        _expandedSections.add(guestId);
        // Notify parent about the guest group change
        widget.onGuestGroupChanged?.call(guestId);
      }
    });
  }
  ...
  Widget _buildGuestInfoHeader(BuildContext context) {
    return Container(
  ...
  Widget _buildGuestSection(
    ...
    final items = _getItemsForGuest(guestId);
    final isExpanded = _expandedSections.contains(guestId);
    final isSelected = _selectedGuestGroup == guestId;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          // Section Header
          InkWell(
            onTap: () => _handleSectionTap(guestId),
            borderRadius: BorderRadius.circular(8),
            child: Container(
              ...
              child: Row(
                children: [
                  ...
                  // Item Count Badge or Active Indicator
            ...
            Container(
              ...
              child: items.isEmpty
                  ? Container(
                      ...
                      child: Center(
                        child: Text(
                          'Tap items to add to $guestLabel',
                          style: TextStyle(
                            fontSize: 12,
                            color: isSelected
                                ? Theme.of(context).colorScheme.primary
            ...
            child: Row(
              ...
              children: [
                if (widget.currentTableName != null) ...[
                  Icon(Icons.table_restaurant, size: 16, color: Colors.white),
                  const SizedBox(width: 6),
                  Text(
                  ...
                  Text(
                    'Dine-In (${widget.orderNumber ?? 'new'})',
                    style: Theme.of(context).textTheme.bodyMedium?.copyWith(
                      color: Colors.white,
                      fontWeight: FontWeight.w600,
                    ),
  ...
  Map<String, dynamic> _formatVoidReceiptData(
    ...
    return {
      ...
      'orderType': widget.orderType?.toUpperCase() ?? 'DINE-IN',
  ...
  void _showContactModal(BuildContext context) {
    ...
    showDialog(
      ...
      builder: (context) => ContactModal(
        ...
        storeName: storeName,
        currentPickupTime: null, // Dine-in orders don't have pickup time
        onCancel: () {
          Navigator.of(context).pop();
        },
