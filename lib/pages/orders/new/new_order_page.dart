...
import 'package:zipzap_pos_self_orders/modals/staff_selector_modal.dart';
...
class _NewOrderPageState extends State<NewOrderPage> {
  ...
  String _orderType = ApiConstants.uiOrderTypeTakeout; // Default to takeout
  ...
  void initState() {
    ...
    _loadData();
    _loadModifiers();
    _initSelectedStaffFromProfile();
  }

  /// Default the assigned server to the currently logged-in user.
  /// Operators can override via the "Select Server" menu in the cart drawer.
  void _initSelectedStaffFromProfile() {
  ...
  void _handleStaffSelect() {
    showStaffSelectorModal(
      context,
      ...
      onConfirm: (staff) {
        ...
        AppToast.success(
          ...
          description: 'Server set to ${staff.fullName}',
        );
      },
    );
  }
  ...
  void _populateFromOrder(order_model.Order order) {
    ...
    // Restore the assigned server (if any) so the cart drawer shows the
    // right name on reopen. Prefer the explicit staff field; fall back to
    // createdBy for older orders that pre-date this feature.
    final restoredStaff = order.staff ?? order.createdBy;
    if (restoredStaff != null && restoredStaff.id.isNotEmpty) {
      _selectedStaff = StaffMember(
        id: restoredStaff.id,
  ...
  Future<void> _handleNoteUpdate(String? note) async {
    ...
    if (_isEditMode && _editingOrderId != null) {
      try {
        ...
        final feeAmount = double.parse(
          (_cartData?.fees.isEmpty ?? true
                  ? 0.0
        ...
        final total = double.parse(
          (discountedTotal + feeAmount + tax).toStringAsFixed(2),
        );
    ...
    try {
      ...
      if (!_isEditMode) {
        ...
        final createdOrder = await _ordersService.createOrder(
          ...
          discount: discount,
          delayTime: delayTime,
          // Assigned server selected via the cart drawer's "Select Server"
          // menu. Defaults to the logged-in user.
        ...
        // Update DataProvider immediately so takeout page shows the new order
        ...
        // For NEW orders, clear cart and redirect to takeout page
        const redirectRoute = '/takeout';
  ...
  Future<void> _handlePrintKitchen() async {
    ...
    try {
      ...
      // Mark items as sent to kitchen if printing was successful
  ...
  Future<void> _handlePickupTimeChanged(DateTime? pickupTime) async {
    setState(() {
      _selectedPickupTime = pickupTime;
    });

    // If in edit mode, immediately update the order with the new pickup time
    if (_isEditMode && _editingOrderId != null) {
      try {
        // Send pickup time as ISO timestamp
        final delayTime = pickupTime?.toIso8601String();
        ...
        if (mounted) {
          AppToast.success(
            ...
            description: pickupTime != null
                ? 'Pickup time set to ${DateFormat('h:mm a').format(pickupTime)}'
                : 'Pickup time cleared',
          );
        }

        debugPrint(
          'Pickup time updated for order $_editingOrderId: $delayTime',
        );
      } catch (e) {
        debugPrint('Error updating pickup time: $e');
        if (mounted) {
          AppToast.error(
            context: context,
            title: 'Failed to Update Pickup Time',
            description: e.toString(),
          );
        }
      }
        ...
        body: Scaffold(
          ...
          endDrawer: isSmallScreen
              ? CartDrawer(
                  ...
                  onPickupTimeTap: () {
                    // TODO: Handle pickup time tap
                    AppToast.info(
                      context: context,
                      title: 'Coming Soon',
                      description: 'Pickup time feature will be available soon',
                    );
                  },
                  onPickupTimeChanged: _handlePickupTimeChanged,
                  initialPickupTime: _selectedPickupTime,
                  ...
                  onStaffSelect: _handleStaffSelect,
                        ...
                        CartDrawer(
                          ...
                          onPickupTimeTap: () {
                            ...
                            AppToast.info(
                              ...
                              title: 'Coming Soon',
                              description:
                                  'Pickup time feature will be available soon',
                            );
                          },
                          onPickupTimeChanged: _handlePickupTimeChanged,
                          initialPickupTime: _selectedPickupTime,
